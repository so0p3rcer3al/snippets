...
public:
	...
	void SetField(U n)
	{
		...
		/* May or may not be faster than regular method --
		 * definitely should be benchmarked. */
		if (fi::width == 1) {
			if (n == 0)
				store = U(store & ~fi::mask);
			else
				store = U(store | fi::mask);
			return;
		}
		 ...
		 * overwrite the contents of adjacent fields. If
		 * this is used, the optimization for width == 1
		 * should be disabled for consistency in invalid
		 * value handling. (Right now, we can just say
		 * it's undefined!) */
